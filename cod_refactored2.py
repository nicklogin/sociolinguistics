...
на операционную систему)'''
    def __init__(self, cat_paths):
        ...
        stopwords = ['я','ты','он','она','оно','они','не','в','п','над','под','от','к',
                 'да','нет','перед','через','после','спереди','сзади','из-за','и',
                 'а','но','для','до']
...
class text_len_stat():
    ...
    def get_stats(self, texts):
        ...
        return {'var':statistics.pvariance(lens),'stdev':statistics.pstdev(lens)}
...
if __name__ == '__main__':
    ...
    most_frequent = a.get_keywords('*',20)
    print(most_frequent)
    detective = a.get_keywords('detective',20)
    average_1 = a.get_corpora_average(detective)
    horror = a.get_keywords('horror',20)
    average_2 = a.get_corpora_average(horror)
    spy = a.get_keywords('spy',20)
    average_3 = a.get_corpora_average(spy)
    comparision = [(detective, average_1), (horror, average_2), (spy, average_3)]
    print(comparision)
